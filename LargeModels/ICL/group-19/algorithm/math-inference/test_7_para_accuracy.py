...
# 处理每个任务的函数
def process_sample(i, sample, group, args, model_name, jsonl_path, data_name):
    global stop_true_no_stop_false, stop_false_no_stop_true, both_true, both_false
    global stop_token_usage, no_stop_token_usage
    ...
    # 带 stop token 的请求
    outputs = completion_request(
        question=action_from_root_stop, args=args, tree_id=i, model_id=model_name,
        n_sampling=1, timeout_sec=100000,max_tokens=24000,
    )
    ...
    # 不带 stop token 的请求
    outputs = completion_request(
        question=action_from_root_no_stop, args=args, tree_id=i, model_id=model_name,
        n_sampling=1, timeout_sec=100000,max_tokens=24000,
    )
...
def run_in_parallel(samples, grouped_data, args, model_name, jsonl_path, data_name):
    # 线程池执行每个样本任务
    task_params = [
        (i, sample, group, args, model_name, jsonl_path, data_name)
        for i, (sample, group) in enumerate(zip(samples, grouped_data.values()))
    ]

    with ThreadPoolExecutor(max_workers=args.max_generate_workers * args.num_of_models) as executor:
        list(tqdm(executor.map(lambda p: process_sample(*p), task_params), total=len(samples), desc="Processing samples"))
...
if __name__ == "__main__":
    ...
    run_in_parallel(samples, grouped_data, args, args.model_name_or_path, jsonl_path, data_name)
    dump_dict = {
            "tree_id": i,
            "stop_true_no_stop_false": stop_true_no_stop_false,
            "stop_false_no_stop_true": stop_false_no_stop_true,
    ...
    #     stop_task_param = dict(
    #         question=action_from_root_stop,
    #         args=args,
    #         tree_id=i,
    ...
    #     )
    #     no_stop_task_param = dict(
    #         question=action_from_root_no_stop,
    #         args=args,
    #         tree_id=i,
    #         model_id=args.model_name_or_path,
    #         n_sampling=1,
    ...
    #         # use_chat=False, stop_tokens=None, next_token=["Yes","No"], ...
    #     )
    #     task_params.append(stop_task_param)
    #     task_params.append(no_stop_task_param)
