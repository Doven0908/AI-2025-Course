...
if __name__ == "__main__":
    ...
    examples = examples.select(range(min(len(examples),15))) 
    ...
    for i,sample in enumerate(samples):
        ...
        while True:
            ...
            chat_resp = chat_completion_request(question=ask_request,args=args,tree_id=i,
                                        need_get_next_token_prob=True,next_token=None,max_tokens=5,n_sampling=1,stop_tokens=["?"]) # ["Yes","No","yes","no"]
            # print(ask_request,"\n\n\n")
            ...
            while(max_n < min(5,len(chat_resp["logits"][0])) and yes_prob + no_prob < 0.1):
                ...
                yes_prob = prob_map.get("yes", 0.0) + prob_map.get("Yes", 0.0) + prob_map.get(" yes", 0.0) + prob_map.get(" Yes", 0.0)
                no_prob  = prob_map.get("no", 0.0) + prob_map.get("No", 0.0) + prob_map.get(" no", 0.0) + prob_map.get(" No", 0.0)
                max_n += 1
            ...
            if yes_prob > no_prob:
                ...
                comp_resp = completion_request(question=get_answer_prompt,args=args,tree_id=i,model_id=args.model_name_or_path,
                                        need_get_next_token_prob=True,stop_tokens=["\n","\n\n"],next_token=None,max_tokens=20,n_sampling=1)
                ...
                llm_judge_equal = None
                if data_name.startswith("olympiadbench") and data_name.startwith("aime"):
                    partial_response = new_action + temp_action
                    llm_judge_equal = llm_judge_once(question=sample[0],ground_truth=sample[2],partial_response=partial_response,args=args,tree_id=i)
                else:
                    llm_judge_equal = True
