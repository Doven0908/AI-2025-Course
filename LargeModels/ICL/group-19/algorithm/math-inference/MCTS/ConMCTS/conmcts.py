...
from .examples import SPLIT_INPUT,SPLIT_PROMPT

from utils.utils import print_time_exceed_warning
from utils.strip_string import extract_answer
from utils.serve_vllm import completion_request,get_token_usage,chat_completion_request
...
def expand(tree:CON_MCTS_Tree,args):
    def get_generation_prompt(node:TreeNode):
        input_prompt = ""
        states = []
        cur_node = node
    ...
    def get_action(node:CON_MCTS_TreeNode,args):
        input_prompt = get_generation_prompt(node)
        n_sampling = args.n_sampling

        model_input = input_prompt
        outputs = completion_request(question=model_input,args=args,tree_id=tree.tree_id,model_id=args.model_name_or_path,n_sampling=n_sampling)
        actions = outputs["texts"][0]
    ...
    def split_action(expand_node,actions,args):
        states = []
        ...
        for i, s in enumerate(states):
            if not s:   # None, 空容器, 0, False, 空张量(若定义了 bool) 等都会触发
                ...
                step_dict.append({"step": i+1, "content":s})
        
        expand_node_states = ""
        expand_node_split_step = ""
        for state in states:
            expand_node_states += state
        for step in step_dict:
            expand_node_split_step += json.dumps(step, ensure_ascii=False) + '\n'
        
        results = []
        for action in actions:
            split_text = SPLIT_INPUT.format(question=tree.question,solution=expand_node_states+action)
            split_input = SPLIT_PROMPT + split_text
            split_request = [[{"role": "user", "content": split_input},
                             {"role": "assistant", "content": expand_node_split_step}]]
            split_output = chat_completion_request(question=split_request,args=args,tree_id=tree.tree_id,model_id=args.model_name_or_path,n_sampling=1)
            result = extract_json_objects(split_output)
    ...
    def recheck_action():
        ...
        actions = get_action(expand_node,args)
        results = split_action(expand_node,actions,args)
